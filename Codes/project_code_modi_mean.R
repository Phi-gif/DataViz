...
plot(Umap.data$layout, col = data[,4], xlab = 'Dim1',ylab = 'Dim2', main = "Paramètres par défaut", pch = 20) # différence de zone
...
for (i in c(0.05, 0.1, 0.25, 0.5, 0.8, 0.99)){
  umap_par$min_dist=i
  U=umap(data[,-c(1,4)],config = umap_par)
  plot(U$layout,col=data[,4],xlab = 'Dim1',ylab = 'Dim2', main = paste("min_dist = ",i), pch = 16)
}
...
for (i in c(2, 5, 10, 20, 50, 100)){
  ...
  U=umap(data[,-c(1,4)],config = umap_par)
  plot(U$layout,col=data[,4],xlab = 'Dim1',ylab = 'Dim2', main = paste("nombre de ppv : ",i),pch=16)
}
